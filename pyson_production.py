...
def decision_tree(data, attributes, target_attribute, algorithm, logBase=10, attributes_used=[]):
    ...
    if not data or (len(attributes) - 1) <= 0:
        ...
        if algorithm == "id3":
            ...
            for value in unique:
                subtree = decision_tree(get_examples(data, best_attr, value),
                                        [attr for attr in attributes if attr != best_attr], target_attribute, algorithm)
...
def best_latent_variable(X, Y, latent_variables, num_instances):
    ...
    for lv in range(1, latent_variables):
        ...
        if num_instances < 10:
            ...
            predY = cross_validation.cross_val_predict(plsca, numpy.array(X), numpy.array(Y),
                                                       cv=num_instances)  #### test
        else:
...
def get_vip(fin_pls, lv_best, current_attribute, attributes_gone, attributes):
    ...
    SS = numpy.power(fin_pls.y_loadings_[0][lv_best - 1], 2) * numpy.matrix.transpose(fin_pls.x_scores_[lv_best - 1]) * \
         fin_pls.x_scores_[lv_best - 1]
...
def plsvip(X, Y, V, lat_var):
    ...
    while attributes > 0:
        ...
        if min_att > -1:
            attributes_gone.append([V[min_att], round(currentR2, 2), attributes,
                                    lv_best])  ####### CURRENT : to BE popped, NOT already popped
        V.pop(min_att)
    ...
    if not data or (len(attributes) - 1) <= 0:
        ...
        if algorithm == "id3":
            ...
            if len(unique) > 1 and best_gain > 0:
                ...
                for value in unique:
                    subtree = decision_tree(get_examples(data, best_attr, value),
                                            [attr for attr in attributes if attr != best_attr], target_attribute,
                                            algorithm)
                    tree[best_attr][value] = subtree
            else:
                tree = default
...
def create_task_lm_train():
    ...
    variables, datapoints, predictionFeature, target_variable_values, parameters = getJsonContentsTrain(
        readThis)  # original
...
def create_task_mci_train():
    ...
    if not (logBase):
        logBase = 10
    if is_number(target_variable_values[0]):
        bin_width = 3.5 * (numpy.std(target_variable_values)) * (
            numpy.power(len(target_variable_values), -0.3))  # Scott
        num_bins = math.ceil(max(target_variable_values) - min(target_variable_values) / bin_width)
        num_bins = int(round(num_bins))
        if num_bins < 2:
            num_bins = 2
